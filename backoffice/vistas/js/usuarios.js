...
"ajax":"ajax/tabla-usuarios.ajax.php?myvar="+GlobalJS,
 	"deferRender": true,
  	"retrieve": true,
  	"processing": true,
	"language": {
...
$(".tablaUsuarios tbody").on("click", "button.btnEliminarUsuario", function(){
	...
	swal({
		...
		if (result.value) {
			...
			window.location = "index.php?pagina=usuarios&idUser="+idUser;
...
$("input[name='altaEmail']").change(function(){
	...
	$.ajax({
		...
		success:function(respuesta){
			...
			if(respuesta){
				...
				$("input[name='altaEmail']").val("");

				$("input[name='altaEmail']").after(`
